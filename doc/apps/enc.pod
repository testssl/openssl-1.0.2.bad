...
use the PBKDF2 algorithm to stretch a password into a key.
SHA1 will be used if no other message digest (using -md) has been specified.
Generate a longer key (modulo keysize) if specified more than once.

=item B<-c number>

the number of iterations that PBKDF2 will use.
