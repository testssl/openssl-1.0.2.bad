...
create and print a key using the PBKDF2 function. Specify the pseudorandom
function using the -md parameter. This function does not do any encryption.

=item B<-iter>

specify the number of rounds for PBKDF2.

=item B<-dklen number>

specify the derived key length for PBKDF2 in bytes.
...
Derive a 128 bit key using PBKDF2, SHA 256 and 4096 rounds using
password "password" and salt "salt"

 openssl enc -pbkdf2 -md sha256 -iter 4096 -pass pass:password -S 73616c74
